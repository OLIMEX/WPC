...
char *argv[];
{
  ...
  if ( !strcmp ( opr , "LIST")) {
   ...
   if (!strncmp ( opr , "INSERT" , 6 )) {
      i = 0 ;
      sscanf ( opr ,"%s %d" , line , &i );
      if ( i <= 0 ) continue ;
      pl = p_buf ;   num = 0 ; l = 0 ;
      while ( *pl ) {
        if ( (i - 1) == num  ) {
   ...
   if (!strncmp ( opr , "DELETE" , 6 )) {
      i = j = 0 ;
      sscanf ( opr ,"%s %d %c %d" , line , &i ,&ch ,  &j );
      if ( i > j && j > 0 ) continue ;
      if ( i <= 0 || j < 0 ) continue ;
      pl = p_buf ;   num = 1 ;
      while ( *pl )
      ...
      // printf ( "pbeg=%s", pbeg) ;
      if ( pend != NULL && pbeg != NULL) {
         memmove ( pbeg , pend , strlen (pend) + 1 ) ;
         l = strlen (p_buf) ;
         if ( p_buf[l-1] == '\n' )
            p_buf[l-1] = '\0' ;
  ...
  if ( strcmp ( opr , "QUIT") && strcmp ( opr , "LIST") && strcmp ( opr , "RUN") &&
       strncmp ( opr , "INSERT" , 6 ) &&  strncmp ( opr , "DELETE" , 6 ))
